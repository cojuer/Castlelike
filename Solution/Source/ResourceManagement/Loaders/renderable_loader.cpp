...
bool RenderableLoader::init(ResourceManager& resManager, RenderSubsystem& rendSubsystem)
{
...
Renderable* RenderableLoader::getSprSheet(PugiNode& node) const
{
    ...
    for (PugiNode frame = node.child("frame"); frame; frame = frame.next_sibling("frame"))
    {
        ...
        Color color{ r, g, b };
        textures[counter++] = m_resManager->textRenderer->renderTexture(text, fName, fSize, color);
    }
